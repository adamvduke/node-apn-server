...
function start() {
	...
	app.get('/users', function(request, response) {
		db.view("users", "all", {include_docs: true}, function(error, result){
			...
			response.render('users/index', { locals: { users: users } });
		});
	});
	...
	app.get('/users/:id', function(request, response) {
		var options = { include_docs: true, startkey_docid: request.params.id, limit: 1 };
		db.view("users", "all", options, function(er, result) {
			response.render('users/show', { locals: { user: result.rows[0].doc } });
		});
	});

	app.get('/users/:id/edit', function(request, response) {
		var options = { include_docs: true, startkey_docid: request.params.id, limit: 1 };
		db.view("users", "all", options, function(er, result) {
			response.render('users/edit', {locals: { user: result.rows[0].doc } });
		});
	});

	app.put('/users/:id', function(request, response) {
	  var id = request.params.id;
	  //TODO: Save the data on the form
...
function resetConnections(db, startServer) {
